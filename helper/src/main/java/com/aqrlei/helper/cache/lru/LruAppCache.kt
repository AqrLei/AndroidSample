...
import com.aqrlei.helper.cache.lru.disklrucache.DiskLruCache
...
class LruAppCache private constructor(
    ...
    init {
        with(lruCacheConfig) {
            memoryCache = object :
                LruCache<String, ByteArray>((CacheFileHelper.runtimeMaxMemory() * memoryCacheRatio).toInt()) {
    ...
    private fun getDiskLruCache(context: Context): DiskLruCache {
        cacheDir = CacheFileHelper.getAppCacheDirFile(context, lruCacheConfig.diskCacheFileName)
        return DiskLruCache.open(
            cacheDir, AppHelper.getAppVersion(context), 1,
